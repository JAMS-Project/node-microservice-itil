...
export const prbModifyField = async (_parent: any, args: IINCModifyFields, context: any): Promise<boolean> => {
  ...
  const findCase = await context.app.mongo.db.collection('prbItems').findOne({ number })
  ...
  // update the case
  await context.app.mongo.db.collection('prbItems').updateOne({ number }, { $set: { ...fields } })

  // @todo RabbitMQ Call to Let Know All Services that want to listen for "itil.prb.modify" action to look at the payload
